...
@interface MCSpecialKeyboardLayout()
/** 特殊字符键盘内容 */
@property (nonatomic, strong) NSMutableArray *specialCharacterTitle;

@end

@implementation MCSpecialKeyboardLayout
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.specialCharacterTitle = [[NSMutableArray alloc] initWithObjects:@"!",@"@",@"#",@"$",@"%",@"^",@"&",@"*",@"(",@")",
                                      @"'",@"\"",@"=",@"_",@":",@";",@"?",@"~",@"|",@"•",
                                      @"+",@"-",@"\\",@"/",@"[",@"]",@"{",@"}",@"delete",
                                      @"123",@",",@".",@"<",@">",@"€",@"£",@"¥",@"ABC",nil];
        [self addSubLayouts];
...
- (void)addFourthRowLayout {
    ...
    [toNumberButton setTitle:self.specialCharacterTitle[29] forState:UIControlStateNormal];
    ...
    toCharacterButton.myWidth = toNumberButton.myWidth;
    toCharacterButton.myHeight = toNumberButton.myHeight;
    ...
    [toCharacterButton setTitle:self.specialCharacterTitle[37] forState:UIControlStateNormal];
